...
export function PageHeader({
  ...
  return (
    <div className={cn("mb-3 flex flex-col gap-3 sm:mb-4 sm:flex-row sm:items-start sm:justify-between", className)}>
      ...
      {actions ? (
        <div className="grid w-full gap-2 [grid-template-columns:repeat(auto-fit,minmax(0,1fr))] sm:flex sm:w-auto sm:shrink-0 sm:flex-wrap sm:items-center [&_a]:h-9 [&_button]:h-9">
          {actions}
        </div>
      ) : null}
    </div>
